...
import { MEASUREMENT_TYPES, OPERATIONS, UNITS_BY_TYPE } from '../constants/units'
...
export default function Calculator() {
  ...
  async function handleSubmit(e) {
    ...
    if (operationType === 'CONVERT' && (!meta.resultUnit || meta.resultUnit === '')) {
      ...
      <form className="calc-layout" onSubmit={handleSubmit}>
        <div className="calc-main card">
          <section className="calc-section" aria-labelledby="meas-label">
            ...
            <div className="calc-seg" role="group" aria-label="Measurement type">
          ...
          <section className="calc-section" aria-labelledby="op-label">
            ...
            <div className="calc-chips" role="group" aria-label="Operation type">
              {OPERATIONS.map((o) => (
                <button
                  key={o.value}
                  type="button"
          ...
          <section className="calc-section">
            ...
            <div className="calc-values">
              ...
              <div className="calc-field">
                ...
                <select id="u1" value={u1} onChange={(e) => setU1(e.target.value)}>
                  {units.map((u) => (
            ...
            {needsTwo && operationType !== 'CONVERT' ? (
              <div className="calc-values calc-values--pair">
                ...
                <div className="calc-field">
                  ...
                  <select id="u2" value={u2} onChange={(e) => setU2(e.target.value)}>
                    {units.map((u) => (
                      <option key={u} value={u}>
                        {u}
                      </option>
            ...
            operationType !== 'DIVIDE' ? (
              <div className="calc-field calc-field--full">
                <label htmlFor="ru">
                  {operationType === 'CONVERT' ? 'Convert to' : 'Result unit (optional)'}
                </label>
                <select
                  id="ru"
                  value={resultUnit}
                ...
                >
                  <option value="">
                    {operationType === 'CONVERT' ? 'Select target unit…' : 'Default (first unit)'}
                  </option>
                  {units.map((u) => (
                    <option key={u} value={u}>
                      {u}
          ...
          {error ? <p className="form-error calc-error">{error}</p> : null}

          <div className="calc-actions">
            <button className="btn-primary calc-run" type="submit" disabled={loading}>
            ...
            {result ? (
              isCompareResponse(result) ? (
                <>
                  <div
                    className={`calc-result-bool ${
                      String(result.unit).toLowerCase() === 'true' ? 'is-true' : 'is-false'
                    }`}
                  >
                    {String(result.unit).toLowerCase() === 'true' ? 'true' : 'false'}
                  </div>
                ...
                <>
                  ...
                  <p className="calc-result-hint">Rounded to two decimals on the server.</p>
                </>
              )
            ) : (
              <p className="calc-result-empty">Run an operation to see the output here.</p>
            )}
          </div>
